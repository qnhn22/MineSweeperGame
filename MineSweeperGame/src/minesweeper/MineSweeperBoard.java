...
public class MineSweeperBoard {
    ...
    public int numAdjMines(int row, int col) {
        ...
        for (int i = row - 1; i < row + 2; i++) {
        	for (int j = col - 1; j < col + 2; j++) {
        		...
        		{
        			if (i != row || j != col) {
        				if (board[i][j] == MINE || board[i][j] == UNCOVERED_MINE || board[i][j] == FLAGGED_MINE) {
        					count++;
        				}
        			}
        		}
    ...
    public void uncoverCell(int row, int col) {
    	if (board[row][col] != FLAG || board[row][col] != FLAGGED_MINE) {
    		if (board[row][col] != MINE && board[row][col] == COVERED_CELL) {
    ...
    public void flagCell(int row, int col) {
    	...
    	else if (board[row][col] == FLAGGED_MINE) {
    		board[row][col] = MINE;
    	}
    }
    ...
    public boolean gameLost() {
    	...
    	for (int row = 0; row < board.length; row++) {
    		for (int col = 0; col < board[row].length; col++) {
    			if (board[row][col] == UNCOVERED_MINE) {
    				isLost = true;
    			}
    		}
    	}
    ...
    public boolean gameWon() {
        boolean isWon = false;
        int numCells = (this.getRows())*(this.getColumns());
        int count = 0;
        for (int row = 0; row < board.length; row++) {
        	for (int col = 0; col < board[row].length; col++) {
        		if (board[row][col] != FLAG && board[row][col] != MINE && board[row][col] != COVERED_CELL) {
        			count++;
        		}
        	}
        }
